...
function ready(datapoints) {
  ...
  }).attr('stroke-width', 2).attr('fill', 'none').attr('class', function (d) {
    return d.key.toLowerCase().replace(/[a-z]/g, '');
  }).classed('highlight-all', true).classed('us', function (d) {
    // console.log(d)
    if (d.key === 'U.S.') {
      return true;
    }
  }).classed('region-highlight', function (d) {
  ...
  svg.selectAll('text').data(nested).enter().append('text').attr('y', function (d) {
    return yPositionScale(d.values[0].price);
  }).attr('x', function (d) {
    return xPositionScale(d.values[0].datetime);
  }).text(function (d) {
    return d.key;
  }).attr('dx', 6).attr('dy', 4).attr('font-size', '12').attr('class', function (d) {
    return d.key.toLowerCase().replace(/[a-z]/g, '');
  }).classed('highlight-all', true).classed('us', function (d) {
    // console.log(d)
    if (d.key === 'U.S.') {
  ...
  svg.append('text').attr('font-size', '24').attr('text-anchor', 'middle').attr('class', 'title').text('U.S. housing prices fall in winter').attr('x', width / 2).attr('y', -40).attr('dx', 40);
  var rectWidth = xPositionScale(parseTime('February-17')) - xPositionScale(parseTime('November-16'));
  svg.append('rect').attr('x', xPositionScale(parseTime('December-16'))).attr('y', 0).attr('class', 'winter-rect').attr('width', rectWidth).attr('height', height).attr('fill', '#C2DFFF').style('visibility', 'hidden').lower();
  ...
  svg.append('g').attr('class', 'axis y-axis').call(yAxis); // Here goes all the steps
  // First: nothing shows up except the axes

  d3.select('#blank-graph').on('stepin', function () {
    svg.selectAll('.highlight-all').attr('fill', 'none').attr('stroke', 'none');
  }); // Second: Everything shows up, except the winter rectangle

  d3.select('#draw-lines').on('stepin', function () {
    svg.selectAll('circle.highlight-all').style('visibility', 'visible').attr('fill', function (d) {
      return colorScale(d.key);
    }).attr('r', 4).attr('cy', function (d) {
      return yPositionScale(d.values[0].price);
    }).attr('cx', function (d) {
    ...
    svg.selectAll('path.highlight-all').transition().attr('stroke', function (d) {
      return colorScale(d.key);
    });
    svg.selectAll('text').style('visibility', 'visible').transition().attr('fill', 'black');
  }); // Third: highlight US lines, circle and text

  d3.select('#highlight-us').on('stepin', function () {
    svg.selectAll('path').transition().attr('stroke', 'lightgray');
    svg.selectAll('circle').transition().attr('fill', 'lightgray');
    svg.selectAll('text.highlight-all').transition().attr('fill', 'lightgray').attr('font-weight', 'bold');
    svg.selectAll('circle.us').transition().attr('fill', 'red'); // .raise()

    svg.selectAll('path.us').transition().attr('fill', 'none').attr('stroke', 'red'); // .raise()

    svg.selectAll('text.us').transition().attr('fill', 'red').attr('font-weight', 'bold'); // .raise()
  }); // Fourth: highlight the lines per region

  d3.select('#highlight-regions').on('stepin', function () {
    svg.selectAll('path').transition().attr('stroke', 'lightgray');
    svg.selectAll('circle').transition().attr('fill', 'lightgray');
    svg.selectAll('.label-text').transition().attr('fill', 'lightgray');
    svg.selectAll('circle.us').transition().attr('fill', 'red');
    svg.selectAll('path.us').transition().attr('fill', 'none').attr('stroke', 'red');
    svg.selectAll('text.us').transition().attr('fill', 'red');
    svg.selectAll('path.region-highlight').transition().attr('stroke', '#B9E3EA');
    svg.selectAll('text.region-highlight').transition().attr('fill', '#B9E3EA');
    svg.selectAll('circle.region-highlight').transition().attr('fill', '#B9E3EA');
  ...
  function render() {
    console.log('Something happened'); // Calculate height/width
    ...
    yPositionScale.range([newHeight, 0]); // Reposition/redraw your elements
    ...
    svg.selectAll('text.highlight-all').attr('y', function (d) {
      return yPositionScale(d.values[0].price);
    }).attr('x', function (d) {
      return xPositionScale(d.values[0].datetime);
    }).attr('font-size', '10').attr('dx', 6).attr('dy', 4);
    svg.select('.title').attr('font-size', '20').attr('x', newWidth / 2).attr('y', -40).attr('dx', 40);
    svg.select('rect').attr('x', xPositionScale(parseTime('December-16'))).attr('width', rectWidth).attr('height', newHeight); // Update axes if necessary
...
},{"d3":"../node_modules/d3/index.js","./data/housing-prices.csv":"data/housing-prices.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
