...
},{}],"data/all-temps.csv":[function(require,module,exports) {
"use strict";
...
var colorScale = d3.scaleOrdinal().range(['#E5FDC0', '#99e6e6', '#957AC1', '#F29482', '#B81D75']).domain(allCities);
...
function ready(datapoints) {
  ...
  var nycData = datapoints.filter(function (d) {
    return d.city === 'NYC';
  });
  nycData.push(nycData[0]);
  container.append('path').attr('class', 'temp').datum(nycData).attr('d', line).attr('fill', colorScale('NYC')).attr('opacity', 0.75);
  ...
  function displayCity(city) {
    ...
    var cityDatapoints = datapoints.filter(function (d) {
      return d.city === city;
    });
    cityDatapoints.push(cityDatapoints[0]);
    container.select('.temp').datum(cityDatapoints).transition().attr('d', line).attr('fill', colorScale(city));
    container.select('.city-name').text(city);
    var labelName = '.label-' + city; // console.log(labelName)
  ...
  function render() {
    ...
    var newRadius = newWidth / 2.5; // Update your SVG
    ...
    radiusScale.range([40, newRadius]); // Reposition/redraw your elements
    ...
    container.selectAll('.temp-notes').attr('x', 0).attr('y', function (d) {
      return -radiusScale(d);
    });
  }
  ...
  render();
  d3.select('#ready-chart-three').on('stepin', function () {
    displayCity('NYC');
  });
...
},{"d3":"../node_modules/d3/index.js","debounce":"../node_modules/debounce/index.js","./data/all-temps.csv":"data/all-temps.csv"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
